...
public class OnlineTurnController {
    ...
    //HELPERS
    Pair<Block,String> pair;
    public OnlineTurnController(Character chara, RoundController round, OrthographicCamera cam, Socket socket){
        this.socket = socket;
        this.round = round;
        this.cam = cam;
        current = chara;
        ...
        texture = ClassicMode.images.targetMove;
    ...
    public void input(OrthographicCamera cam2){
        ...
        if (MyGestures.isTap()){
            ...
            cam.unproject(vec);
            for(Pair<Block,String> pair : possibleMoves){
                if(pair.getBlock().touched(vec)){ //MOVEMENT HAPPENS
                    dice.setNumber(dice.getValue() - 1);
                    sendData("dice");
    ...
    public void update(){
        ...
        if(dice.rolled && current.moves == 0 && (abilityButton.used || !current.canAbility() && !current.moving)){
            finish = true; // END TURN BECAUSE NO MORE MOVES LEFT
    ...
    public void drawHUD(SpriteBatch batch){
        ...
        batch.draw(dice.movesBg, MainGame.width/2-10,MainGame.height*4/5-10);
        batch.draw(dice.movesImage, MainGame.width/2,MainGame.height*4/5);
        surrenderButton.draw(batch);
    }

    public void sendData(String action){
        if(action.equals("dice")){
            JSONObject data = new JSONObject();
            try{
                data.put("number",dice.getValue());
                socket.emit("updateDice", data);
            }
            catch(JSONException e){
                Gdx.app.log("Socket.IO", "Error sending updateDice");
            }
        }
            ...
            JSONObject data = new JSONObject();
            try{
                data.put("dir",pair.getString());
                data.put("blockX",pair.getBlock().pos_x);
                data.put("blockY",pair.getBlock().pos_y);
                socket.emit("moveCharacter", data);
            }
            catch(JSONException e){
                Gdx.app.log("Socket.IO", "Error sending movement");
            }
        }
    }
